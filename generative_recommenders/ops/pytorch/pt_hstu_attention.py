...
def _get_key_chunk_size(
    default_key_chunk_size: int,
    device: torch.device,
    key_len: int,
) -> int:
    key_chunk_size = default_key_chunk_size
    if key_chunk_size <= 0 and device.type == "npu":
        key_chunk_size = 128
    if key_chunk_size <= 0:
...
def _accumulate_attn_output(
        attn: torch.Tensor,
        qk_attn: torch.Tensor,
        v: torch.Tensor,
) -> None:
        """
        In-place accumulate attn += einsum('bhxy,bhyv->bhxv', qk_attn, v)
        using baddbmm_ to minimize temporary allocations.
        Shapes:
            attn:    [B, H, X, V]
            qk_attn: [B, H, X, Y]
            v:       [B, H, Y, V]
        """
        b, h, x, y = qk_attn.shape
        _, _, y2, vdim = v.shape
...
def pytorch_hstu_mha(
    ...
    if chunk_size > 0 and chunk_size < max_seq_len:
        ...
        for start in range(0, max_seq_len, chunk_size):
            ...
            key_chunk_size = _get_key_chunk_size(
                default_key_chunk_size=int(
                    os.environ.get("HSTU_ATTN_KEY_CHUNK_SIZE", "0")
                ),
                device=q.device,
